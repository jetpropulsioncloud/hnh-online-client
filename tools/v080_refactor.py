...
building=r'''  function musterClassSlug(value){return String(value||'muster').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');}
...
'''
s,n=re.subn(r'  function buildingCard\(p,pi,b\)\{.*?\n  \}\n\n  function workshopAction',building+'\n  function workshopAction',s,count=1,flags=re.S)
assert n==1,'buildingCard replacement failed'

critter=r'''  function critterCard(p,pi,r){
...
'''
s,n=re.subn(r'  function critterCard\(p,pi,r\)\{.*?\n  \}\n\n  function handPanel',critter+'\n  function handPanel',s,count=1,flags=re.S)
assert n==1,'critterCard replacement failed'

combat=r'''  function combatPanel(){
...
'''
s,n=re.subn(r'  function combatPanel\(\)\{.*?\n  \}\n\n  function reactionTray',combat+'\n  function reactionTray',s,count=1,flags=re.S)
assert n==1,'combatPanel replacement failed'

start=s.index('  function installDirectManipulation(){')
end=s.index('\n\n  function render(){',start)
direct=r'''  const combatLinks=document.createElementNS('http://www.w3.org/2000/svg','svg');
...
s=s.replace('Client v0.7.9 · Rules v0.6.2','Client v0.8.0 · Rules v0.6.2').replace("version:'0.7.9'","version:'0.8.0'")
needle='${handPanel()}${drawerPanel()}${harvestOverlay()}${winnerOverlay()}</div>`;\n  }'
assert needle in s,'render tail missing'
s=s.replace(needle,'${handPanel()}${drawerPanel()}${harvestOverlay()}${winnerOverlay()}</div>`;\n    requestAnimationFrame(syncCombatArrows);\n  }',1)
s=s.replace('  installDirectManipulation();\n  render();',"  installDirectManipulation();\n  window.addEventListener('resize',()=>requestAnimationFrame(syncCombatArrows));\n  window.addEventListener('scroll',()=>requestAnimationFrame(syncCombatArrows),{passive:true});\n  render();",1)
client.write_text(s)

css=Path('styles.css')
c=css.read_text()+r'''

/* ===== v0.8.0 combat links + Muster identity ===== */
.musterIdentity{margin:7px 0 5px;padding:7px 9px;border-radius:8px;border:1px solid rgba(62,72,49,.22);display:flex;align-items:baseline;justify-content:space-between;gap:8px;background:rgba(226,215,166,.55)}
.musterIdentity strong{font-family:Georgia,serif;font-size:18px;line-height:1;letter-spacing:.02em}.musterIdentity small{font-size:9px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;opacity:.82;white-space:nowrap}
.muster-scurry{background:rgba(238,190,125,.56)}.muster-lantern{background:rgba(246,218,116,.62);border-color:rgba(153,116,35,.4)}.muster-bramble{background:rgba(167,198,134,.58)}.muster-handwork{background:rgba(204,178,140,.58)}.muster-gatewatch{background:rgba(154,186,194,.58)}.muster-burrow{background:rgba(185,162,133,.62)}
.gameCard.building[data-muster-class] .buildingArt small{font-size:11px;font-weight:900;letter-spacing:.12em;color:#41553a}.gameCard.building[data-muster-class] .cardFrame>p{margin-top:5px}
.combatLinkOverlay{position:fixed;inset:0;width:100vw;height:100vh;z-index:48;pointer-events:none;overflow:visible}.combatAttackPath,.combatBlockPath{fill:none;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 2px 2px rgba(0,0,0,.28))}.combatAttackPath{stroke:#d9783c;stroke-width:5;opacity:.9}.combatBlockPath{stroke:#58a9df;stroke-width:5;opacity:.95}.combatAttackHead{fill:#d9783c}.combatBlockHead{fill:#58a9df}
.dragAttackArrow,.dragBlockArrow{position:fixed;inset:0;width:100vw;height:100vh;z-index:90;pointer-events:none;overflow:visible}.dragAttackArrow line,.dragBlockArrow line{stroke-width:6;stroke-linecap:round;opacity:.78}.dragAttackArrow line{stroke:#d9783c}.dragAttackArrow path{fill:#d9783c}.dragBlockArrow line{stroke:#58a9df}.dragBlockArrow path{fill:#58a9df}.dragAttackArrow.valid line,.dragBlockArrow.valid line{stroke-width:8;opacity:1}
.blockDraggable{cursor:crosshair;box-shadow:0 0 0 2px rgba(88,169,223,.26)}.blockHint{background:rgba(88,169,223,.16)!important;color:#245d7e!important}.compactCombat .reactionTray{margin-top:7px}
'''
css.write_text(c)

idx=Path('index.html');idx.write_text(idx.read_text().replace('v0.7.9','v0.8.0').replace('v=079','v=080'))
readme=Path('README.md')
if readme.exists():readme.write_text(readme.read_text().replace('v0.7.9','v0.8.0'))

test=Path('tests/ui-coordinator-test.js');t=test.read_text();marker="console.log('\\nAll consolidated UI/runtime tests passed.');";assert marker in t
insert=r'''

ok('v0.8.0 uses persistent attack/block arrows and direct blocking',()=>{
  const source=fs.readFileSync('client.js','utf8');
  assert(/combatLinkOverlay/.test(source));assert(/combatBlockPath/.test(source));assert(/blockDraggable/.test(source));assert(/E\.assignBlock\(game,g\.defenderIndex,g\.uid,attackIndex\)/.test(source));
});
ok('v0.8.0 gives Muster Class a prominent identity band',()=>{
  const source=fs.readFileSync('client.js','utf8');assert(/musterIdentity/.test(source));assert(/musterRulesText/.test(source));assert(/data-muster-class/.test(source));
});
'''
test.write_text(t.replace(marker,insert+'\n'+marker))
